...
class InputField:
    ...
    def handleKeyPress(self, event):
        ...
        if event.key == pg.K_BACKSPACE:
            ...
            if (
                str(pg.key.name(event.key)).isalnum()
                and len(str(pg.key.name(event.key))) == 1
            ):
...
class ServerSelect(ViewController):
    def __init__(self):
        ...
        self.IPLabel = Label("IP: 127.0.0.1", self.font)
        ...
        self.submitButton = Button(
            submitPanel, submitLabel, self.palette["yellow"], self.palette["red"]
        )
    ...
    def shouldAdvance(self, controller):
        ...
        if self.ready:
            ...
            controller.socket.connect(("127.0.0.1", portNumber))
            return True
        return False
